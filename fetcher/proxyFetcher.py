...
class ProxyFetcher(object):
    ...
    def freeProxy01():
        ...
        if interval.seconds < 300:  # 只采集5分钟内的更新
            ...
            while target_url:
                ...
                target_url = (
                    f"https://www.zdaye.com/{next_page[0].strip()}"
                    if next_page
                    else False
                )
                sleep(5)
    ...
    def freeProxy05(page_count=1):
        ...
        for page_index in range(1, page_count + 1):
            url_list.extend(pattern.format(page_index) for pattern in url_pattern)
        for url in url_list:
            tree = WebRequest().get(url).tree
    ...
    def freeProxy09(page_count=1):
        ...
        for i in range(1, page_count + 1):
            url = f'http://ip.jiangxianli.com/?country=中国&page={i}'
            html_tree = WebRequest().get(url).tree
            for index, tr in enumerate(html_tree.xpath("//table//tr")):
                if index == 0:
                    continue
                yield ":".join(tr.xpath("./td/text()")[:2]).strip()
